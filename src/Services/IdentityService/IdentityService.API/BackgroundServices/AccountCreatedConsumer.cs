...
namespace IdentityService.BackgroundServices
{
    ...
    /// og sørger for at sync'e accounts ind i IdentityService's egen database.
    /// </summary>
    public class AccountCreatedConsumer : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            await _kafkaConsumer.ConsumeAsync<AccountCreated>(
                ...
                {
                    _logger.LogInformation(
                        "Received AccountCreated event in IdentityService. AccountId={AccountId}, Email={Email}, CreatedAt={CreatedAt}",
                        message.AccountId,
                        message.Email,
                        message.CreatedAt
                    );
