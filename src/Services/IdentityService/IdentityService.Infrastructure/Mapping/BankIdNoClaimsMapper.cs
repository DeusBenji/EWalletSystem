...
/// <summary>
/// Claims mapper for Norwegian BankID.
/// Extracts ONLY age verification status from session data.
/// Uses dateOfBirth from Signicat, DOES NOT parse fødselsnummer.
/// </summary>
public class BankIdNoClaimsMapper : IClaimsMapper
{
    private readonly ISafeLogger<BankIdNoClaimsMapper> _safeLogger;
    private readonly TimeProvider _timeProvider;

    public BankIdNoClaimsMapper(ISafeLogger<BankIdNoClaimsMapper> safeLogger, TimeProvider timeProvider)
    {
        _safeLogger = safeLogger;
        _timeProvider = timeProvider;
    }

    public string ProviderId => "nbid";
    ...
    public AgeVerificationDto MapToAgeVerification(SessionDataDto sessionResponse)
    {
        ...
        if (sessionResponse.Subject?.DateOfBirth == null)
        {
            ...
            throw new AgeVerificationException(
                ...
                "Norwegian BankID requires dateOfBirth attribute");
        }
